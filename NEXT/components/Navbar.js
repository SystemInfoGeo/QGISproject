...
const handleLogout = async () => {
  const isLoggedOut = await logout(); // Attendre la déconnexion de l'utilisateur
  if (isLoggedOut) {
      setMessage('Déconnexion réussie !'); // Définir le message de confirmation
      setTimeout(() => {
          router.push('/login'); // Redirigez vers la page de connexion après un délai
              ...
              <ul className="dropdown-menu" aria-labelledby="navbarDropdown">
                ...
                <li>
                  {loggedIn ? (
                    ...
                    <a className="dropdown-item" onClick={() => router.push('/login')} style={{ cursor: 'pointer' }}>
