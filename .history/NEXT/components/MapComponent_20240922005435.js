...
const MapComponent = ({
  ...
  const [collectedPoints, setCollectedPoints] = useState(Array(points.length).fill(false));
  ...
  const handleMarkerClick = async (index) => {
    ...
    if (!binIds || !binIds[index]) {
      ...
      setErrorMessage(`ID de la poubelle introuvable pour ce point avec l'index ${index}`);  // Affiche le message d'erreur dans l'interface utilisateur
      return;
    }
