...
interface ItinerairesORS {
  routes: { geometry: string }[]; // Structure d'un itinéraire venant de flask
}
...
// Décoder les polylignes encodées et les transformer en LatLngTuple[]
const decodedPolylines: LatLngTuple[][] = itinerairesORS
  .filter(itineraireEncoded => itineraireEncoded && itineraireEncoded.routes) // Vérification si itineraireEncoded et routes existent
  .map(itineraireEncoded => {
    return itineraireEncoded.routes.map(route => {
      if (route && route.geometry) { // Vérification si route et geometry existent
        return polyline.decode(route.geometry).map(coord => [coord[0], coord[1]] as LatLngTuple);
      }
      return []; // Retourner un tableau vide si la géométrie n'est pas définie
    });
  })
  .flat(); // Cela produit un tableau de polylignes décodées.
    ...
    <MapContainer center={center} zoom={10} style={{ height: "100%", width: "100%" }}>
      ...
      {decodedPolylines.map((polylineCoords, index) => (
        <React.Fragment key={`polyline-${index}`}>
          <Polyline positions={polylineCoords as LatLngTuple[]} color="blue" />
