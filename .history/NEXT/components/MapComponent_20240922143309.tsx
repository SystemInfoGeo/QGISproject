...
const MapComponent: React.FC<MapProps> = ({ points = [], optimalPath = [], itinerairesORS = [], onAllCollected, resetMap }) => {
  ...
  const decodedPolylines: LatLngTuple[][] = itinerairesORS.map(itineraireEncoded => {
    return itineraireEncoded.routes.map(route => polyline.decode(route.geometry).map(coord => [coord[0], coord[1]] as LatLngTuple));
