...
/*/////////////////////////// ce code affiche le chemin et tt faits +ieurs étiquettes ça dérange!!!!!!!!!!!!!!!!!!!!!!
import React from 'react';
...
/* ce code a trés bien marché juste que le point d'arrivé et de départ n'est pas trop claire */
/*import React from 'react';
import { MapContainer, TileLayer, Marker, Tooltip, Polyline } from 'react-leaflet';
import { LatLngTuple, divIcon } from 'leaflet';
import 'leaflet/dist/leaflet.css';
...
const MapComponent: React.FC<MapProps> = ({ points, optimalPath }) => {
  ...
  const createIcon = (number: number) => {
    return divIcon({
      html: `<div style="background-color: red; color: black; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;">${number}</div>`,
      className: ''
    });
  };
...
import React from 'react';
import { MapContainer, TileLayer, Marker, Polyline, divIcon } from 'react-leaflet';
import { LatLngTuple } from 'leaflet';
import 'leaflet/dist/leaflet.css';
...
const MapComponent: React.FC<MapProps> = ({ points, optimalPath }) => {
  ...
  // Function to create a custom icon with a number
  const createIcon = (number: number, color: string = 'blue') => {
    return divIcon({
      html: `<div style="background-color: ${color}; color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;">${number}</div>`,
      className: ''
    });
  };
    ...
    <MapContainer center={center} zoom={10} style={{ height: "600px", width: "100%" }}>
      ...
      {optimalPath.map((coord, index) => (
        <Marker
          key={`optimal-${index}`}
          position={coord as LatLngTuple}
          icon={createIcon(index === 0 || index === optimalPath.length - 1 ? 1 : index + 1, index === 0 || index === optimalPath.length - 1 ? 'red' : 'blue')}
        />
      ))}
      {optimalPath.length > 0 && (
        <Polyline
