...
const MapComponent = ({ points = [], optimalPath = [], collectedBins, setCollectedBins }) => {
  ...
  const formattedPoints = points.map(point => ({
    ...
    longitude: point[1],
    status: point.status // Ajoutez le statut ici pour chaque poubelle
  }));
    ...
    <MapContainer center={Point_depart_fixe} zoom={13} style={{ height: '500px', width: '100%' }}>
      ...
      {formattedPoints.map((point, index) => (
        <Marker
          ...
          icon={divIcon({
            html: `<div style="background-color: ${point.status === 'plein' ? 'red' : 'green'}; padding: 5px; border-radius: 50%">${index + 1}</div>` // Utilise le statut pour changer la couleur
          })}
