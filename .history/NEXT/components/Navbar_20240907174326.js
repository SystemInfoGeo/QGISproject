...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (token) {
      setLoggedIn(true); // Si un token existe, l'utilisateur est considéré comme connecté
    } else {
      setLoggedIn(false); // Sinon, il est considéré comme non connecté
    }
  }, []);
  ...
  const handleLogout = () => {
    ...
    fetch(`${apiUrl}/logout`, { method: 'POST' })  // Déconnexion côté serveur
      .then((res) => res.json())
      .then((data) => {
