...
const MapComponent: React.FC<MapProps> = ({ points, optimalPath }) => {
  ...
  const createIcon = (number: number) => {
    return divIcon({
      html: `<div style="background-color: red; color: black; border-radius: 90%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;">${number}</div>`,
      className: ''
    });
  };
