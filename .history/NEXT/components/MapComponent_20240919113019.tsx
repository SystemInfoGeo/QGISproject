...
useEffect(() => {
  // Filtrer les points rouges (pleins) et exclure le point de départ (index 0) et le point d'arrivée (dernier index)
  const filteredPoints = collectedPoints.slice(1, collectedPoints.length - 1); // Ne garder que les points intermédiaires
  
  // Vérifier que tous les points rouges (pleins) sont collectés
  const allCollected = filteredPoints.every(point => point === true);

  console.log('État actuel des points rouges collectés (sans départ et arrivée):', filteredPoints);
  console.log('Tous les points rouges pleins collectés :', allCollected);

  // Activer le bouton seulement si tous les points rouges (pleins) sont collectés
  if (allCollected) {
