...
const MapComponent = ({ points = [], collectedBins, setCollectedBins }) => {
  ...
  // Vérifiez que points est bien un tableau et qu'il contient des coordonnées valides
  ...
  return (
    <MapContainer center={Point_depart_fixe} zoom={13} style={{ height: '800px', width: '100%' }}>
      ...
      {/* Marqueurs des points de collecte avec coordonnées valides */}
      ...
      {/* Tracé de la polyline entre les points avec coordonnées valides */}
      <Polyline positions={[Point_depart_fixe, ...validPoints.map(p => [p.latitude, p.longitude]), Point_arrivee_fixe]} color="blue" />
    </MapContainer>
  );
};
