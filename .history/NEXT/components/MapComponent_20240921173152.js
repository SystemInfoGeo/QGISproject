...
const MapComponent = ({ points = [], optimalPath = [], collectedBins, setCollectedBins }) => {
  ...
  const handleMarkerClick = async (index) => {
    ...
    try {
      const response = await fetch(`/api/updateStatus/${binId}`, { // Utilisation des backticks ici
        method: 'PUT',
    ...
    <MapContainer center={Point_depart_fixe} zoom={13} style={{ height: '500px', width: '100%' }}>
      ...
      {points.map((point, index) => (
        <Marker
          ...
          icon={divIcon({
            html: `<div style="background-color: ${point.status === 'plein' ? 'red' : 'green'}; padding: 5px; border-radius: 50%">${index + 1}</div>` // Utilisation des backticks ici
          })}
