...
// Interface de MapProps avec binIds inclus
interface MapProps {
  points?: Point[];
  ...
  setCollectedBins: React.Dispatch<React.SetStateAction<number[]>>;
  binIds: number[]; // Ajoutez cette ligne pour binIds
}

const MapComponent: React.FC<MapProps> = ({ points = [], optimalPath = [], itinerairesORS = [], onAllCollected, resetMap, collectedBins, setCollectedBins, binIds }) => {
  ...
  const decodedPolylines = itinerairesORS.map(itineraire => {
    return itineraire.routes.map(route => polyline.decode(route.geometry).map(coord => [coord[0], coord[1]] as LatLngTuple));
  }).flat();
    ...
    <MapContainer center={center} zoom={10} style={{ height: "100%", width: "100%" }}>
      ...
      {decodedPolylines.map((polylineCoords, index) => (
        <React.Fragment key={`polyline-${index}`}>
          ...
          <AddPolylineDecorator polyline={polylineCoords} />
        </React.Fragment>
      ))}
