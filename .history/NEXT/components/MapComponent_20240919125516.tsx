...
useEffect(() => {
  // On utilise `optimalPath` qui contient les points calculés par Flask (chemin optimal)
  const initialCollectedPoints = Array(optimalPath.length).fill(false); // Initialisation de collectedPoints avec le chemin optimal
  setCollectedPoints(initialCollectedPoints);
  console.log('Initialisation de collectedPoints pour les points rouges du chemin optimal:', initialCollectedPoints);
}, [optimalPath]); // Utilisez optimalPath ici

// Vérifier les points du chemin optimal collectés
useEffect(() => {
  // Exclure le point de départ et d'arrivée
  const allCollected = collectedPoints.slice(1, -1).every(point => point === true);
  console.log('État actuel des points rouges du chemin optimal collectés:', collectedPoints);
  console.log('Tous les points rouges du chemin optimal pleins collectés :', allCollected);
    ...
    const handleMarkerClick = async (index: number) => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/update-trash-bin-status/${binId}`, {
          ...
          body: JSON.stringify({
            status: 'vide',  // Mettre à jour le statut de la poubelle à 'vide'
          })
        });
