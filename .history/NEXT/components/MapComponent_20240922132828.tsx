...
interface ItinerairesORS {
  routes: { geometry: string }[]; // Structure d'un itinéraire
}
...
const MapComponent: React.FC<MapProps> = ({ points = [], optimalPath = [], itinerairesORS = [], onAllCollected, resetMap }) => {
  ...
  const decodedPolylines = itinerairesORS.map(itineraire => {
    return itineraire.routes.map(route => {
      if (typeof route.geometry === 'string') {
        return polyline.decode(route.geometry).map(coord => [coord[0], coord[1]] as LatLngTuple);
      } else {
        console.error('Geometry is not a string:', route.geometry);
        return []; // Retourne un tableau vide si la géométrie n'est pas une chaîne
      }
    }).flat();
  }).flat();
