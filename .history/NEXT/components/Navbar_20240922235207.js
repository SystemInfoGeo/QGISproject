...
export default function Navbar() {
  ...
  const { loggedIn, setLoggedIn } = useAuth(); // Assurez-vous que setLoggedIn est disponible
  ...
  const handleReportRedirect = () => {
    if (!loggedIn) {
      localStorage.setItem('redirectUrl', '/user/report'); // Stocker l'URL de redirection
      console.log('Redirection vers la page de connexion');
      router.push('/login'); // Rediriger vers la page de connexion
    } else {
      console.log('Redirection vers la page de signalement');
      router.push('/user/report'); // Rediriger vers le formulaire de signalement
    }
  };
  ...
  useEffect(() => {
    if (loggedIn) {
      ...
      if (redirectUrl) {
        ...
        localStorage.removeItem('redirectUrl'); // Supprimer l'URL après la redirection
        router.push(redirectUrl); // Rediriger vers l'URL stockée
      }
    }
  }, [loggedIn, router]);
