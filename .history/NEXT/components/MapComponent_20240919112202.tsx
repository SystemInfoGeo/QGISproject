...
useEffect(() => {
  // Exclure le point de départ (index 0) et le point d'arrivée (dernier index)
  // Ici, on part du principe que collectedPoints contient les statuts des points rouges seulement (points pleins)
  const pointsToCheck = collectedPoints.slice(1, collectedPoints.length - 1); // Ne garder que les points intermédiaires

  // Vérifier que tous les points rouges (pleins) intermédiaires sont collectés (passés à true)
  const allCollected = pointsToCheck.every(point => point === true);
  
  console.log('État actuel de collectedPoints (sans départ et arrivée):', pointsToCheck);
  console.log('Tous les points pleins collectés :', allCollected);

  // Activer le bouton seulement si tous les points rouges (pleins) intermédiaires sont collectés
  if (allCollected) {
    onAllCollected(true);  // Active le bouton
  } else {
