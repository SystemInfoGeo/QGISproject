...
const MapComponent: React.FC<MapProps> = ({
  ...
  const [collectedPoints, setCollectedPoints] = useState<boolean[]>(Array(points.length).fill(false));
   ...
   useEffect(() => {
    console.log("Initialisation des collectedPoints:", collectedPoints);
  }, [])

  useEffect(() => {
    setCollectedPoints(Array(points.length) .fill(false));
    console.log('Reset des collectedPoints:', Array(points.length).fill(false));
  }, [points]);

  useEffect(() => {
     // Vérifier que tous les points intermédiaires sont collectés (ignorer le point de départ et d'arrivée)
    const allCollected = collectedPoints.slice(1, collectedPoints.length - 1).every(point => point);
    console.log('État actuel de collectedPoints:', collectedPoints);
    console.log('tous les points collectés : ', allCollected);
        // Ne pas passer "true" dès le début, attendre que tous les points soient cliqués
        if (!collectedPoints.includes(false)) {
