...
const Login = () => {
  ...
  useEffect(() => {
    // Vérifie si un token existe déjà dans le localStorage et le récupère
    const storedToken = localStorage.getItem('token');
    if (storedToken) {
      setToken(storedToken);  // Si un token est déjà stocké, on le récupère
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const accessToken = response.data.access_token;  // Récupérer le jeton
      localStorage.setItem('token', accessToken);  // Stocker le jeton dans localStorage
      setToken(accessToken);  // Stocker le token dans le state pour l'afficher
  ...
  // Fonction pour afficher le token depuis le localStorage
  const showToken = () => {
    const storedToken = localStorage.getItem('token');
    if (storedToken) {
      console.log('Jeton JWT:', storedToken);
    } else {
