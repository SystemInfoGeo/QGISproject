...
export default function Carte() {
  ...
  const [message, setMessage] = useState(null); // Message d'information
  const [resetMap, setResetMap] = useState(false);
  const [points, setPoints] = useState([]); // Points de collecte
  const [binIds, setBinIds] = useState([]); // IDs des poubelles
  const [collectedBins, setCollectedBins] = useState([]); // Poubelles collectées
  ...
  const handleFinish = async () => {
    if (allCollected) {
      try {
        //Afficher le message de succés
        setMessage('Bravo, la collecte est terminée!');

        // Réinitialiser la carte après un délai de 3 secondes
      setTimeout(() => {
        setResetMap(true); // Réinitialise la carte
        setMessage(null); // Supprimer le message après la réinitialisation
      }, 3000); // 3 secondes de délai avant la réinitialisation
    } catch (error) {
      // Gérer les erreurs ici
      console.error("Erreur lors de la finalisation de la collecte : ", error);
      setMessage('Erreur lors de la finalisation de la collecte.');
    } finally {
      // Tout nettoyage ou action finale si nécessaire
    }
  } else {
