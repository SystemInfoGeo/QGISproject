    ...
    const fetchTrashBins = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Extraire les points de collecte avec leurs latitudes, longitudes
          const points = result.map((point: any) => ({
            latitude: point.latitude,
            longitude: point.longitude,
          }));

          // Extraire les IDs des poubelles
          const binIds = result.map((point: any) => point.id); // Utilisez 'id' pour les binIds

          // Mettre à jour les états avec les points et bin_ids
          setPoints(points);
          setBinIds(binIds);
        } else {
