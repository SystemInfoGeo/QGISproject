...
export default function Carte() {
  ...
  const fetchPoints = async () => {
    try {
        const response = await fetch('http://localhost:5000/trash-status');  // Appel à votre API pour récupérer les points
        if (!response.ok) {
            throw new Error(`Erreur de requête : ${response.statusText}`);
        }
        const data = await response.json();
        
        // Transformer les données pour inclure le statut dans les coordonnées
        const coordinates = data.map(point => [point.longitude, point.latitude, point.status]);
        
        // Envoyer les données filtrées au backend pour calculer le chemin
        const routeResponse = await fetch('http://localhost:5000/calculate-route', {
            method: 'POST',
