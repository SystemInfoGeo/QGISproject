    ...
    const fetchUpdateStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // S'assurer que geometry est bien une chaîne de caractères
          const itinerairesORS = result.itineraires_ors.flatMap((itinerary: { routes: { geometry: string }[] }) =>
            itinerary.routes.map((route) => {
              if (typeof route.geometry === 'string') {
                return route.geometry;
              }
              return null; // ou handlez l'erreur ici
            }).filter(Boolean) // Supprimez les entrées null ou undefined
          );
          ...
          setData({
            ...result,
            itineraires_ors: itinerairesORS, // Assurez-vous que l'itineraires_ors est correctement mappé
          });
        } else {
