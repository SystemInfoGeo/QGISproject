...
export default function Carte() {
  ...
  const fetchPoints = async () => {
    try {
      const response = await fetch('http://localhost:5000/trash-status');
      if (!response.ok) {
        throw new Error(`Erreur de requête : ${response.statusText}`);
      }
      const data = await response.json();
      console.log('Données reçues des poubelles:', data);

      // Traitez les données et envoyez-les pour calculer le chemin
