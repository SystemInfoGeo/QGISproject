...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const totalUsersRes = await axios.get('/total-users');
        setTotalUsers(totalUsersRes.data.total_users);

        const activeUsersRes = await axios.get('/active-users');
        setActiveUsers(activeUsersRes.data.active_users);

        const newUsersRes = await axios.get('/new-users');
        setNewUsers(newUsersRes.data.new_users);

        const messagesRes = await axios.get('/messages');
