...
export default function ManageTrashBins() {
  ...
  useEffect(() => {
    // Fetch all trash bins when component mounts
  ...
  const handleSubmit = async (e) => {
    ...
    setLongitude('');
    setStatus('vide');
    fetchTrashBins();  // Refresh the list
    setTimeout(() => setMessage(''), 3000);  // Clear message after 3 seconds
  };

  const handleEdit = (bin) => {
    // Pre-fill the form with the trash bin data for editing
    setName(bin.name);
  ...
  return (
    <AdminLayout>
      <div className="container mt-5">
        ...
        {/* Formulaire pour ajouter/modifier un point de collecte */}
