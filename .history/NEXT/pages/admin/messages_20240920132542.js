     ...
     /* setLoadingMessages(true); // Activer l'indicateur de chargement des messages*/
      ...
      try {
        const response = await axios.get(`http://127.0.0.1:5000/messages`, {
          headers: {                      
            Authorization: `Bearer ${localStorage.getItem('token')}`, // Ajouter le token JWT dans les en-têtes
          },
        });
       ...
       /* console.log("Réponse complète de l'API :", response.data);
              // Ajoutez ce log pour vérifier le contenu des messages reçus
        console.log("Messages reçus depuis l'API :", response.data.messages);*/
