...
const UserProfile = () => {
  ...
  const [loadingMessages, setLoadingMessages] = useState(false); // Gère l'état de chargement des messages
  ...
  const fetchUserMessages = async () => {
    ...
    setLoadingMessages(true); // Démarrer le chargement des messages
    ...
    try {
      ...
      setMessages(response.data.messages || []); // Vérifiez si "messages" existe dans les données
    } catch (err) {
      console.error('Erreur lors du chargement des messages :', err.response || err);
      setError('Erreur lors du chargement des messages.');
    } finally {
      setLoadingMessages(false); // Arrêter le chargement des messages
    }
  };

  useEffect(() => {
    if (user && showMessages && messages.length === 0) {
      fetchUserMessages();
    }
  }, [user, showMessages]);
            ...
            {user && (
              <>
                ...
                <Button 
                  variant="primary" 
                  onClick={() => {
                    if (!user) return; // S'assurer que le profil est chargé
                      ...
                      <Table striped bordered hover>
                        ...
                        <tbody>
                          {Array.isArray(messages) && messages.length > 0 ? (
                            ...
                            <tr>
                              <td colSpan="5" className="text-center">Aucun message trouvé.</td>
                            </tr>
                          )}
                        </tbody>
                      </Table>
