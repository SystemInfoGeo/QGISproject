...
const UserProfile = () => {
  ...
  // Utilisation de useEffect pour charger les messages lorsque l'utilisateur est chargé et que l'affichage est activé
  useEffect(() => {
    if (user && user.email && showMessages && messages.length === 0) {
      fetchUserMessages();
    }
  }, [user, showMessages]);
