...
const ReportForm = () => {
  const { loggedIn } = useAuth(); // Vérification de l'état de connexion
  const router = useRouter();

  useEffect(() => {
    if (!loggedIn) {
      // Stocker l'URL actuelle dans sessionStorage avant de rediriger
      sessionStorage.setItem('redirectAfterLogin', '/user/report');
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetch('http://localhost:5000/report-bin', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ name, lat, lng, status, comments }),
    });
  ...
  return (
    <div>
      <Navbar /> {/* Intégrer la barre de navigation ici */}
