...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { token, role } = response.data; // Utilisez "token" ici

      // Stocker le token
      localStorage.setItem('access_token', token); // Stockez "token" dans localStorage
      console.log('Token stocké:', localStorage.getItem('access_token'));

      // Stocker le rôle dans localStorage
      localStorage.setItem('role', role);

      // Passer le token au contexte d'authentification
      login(token);
      setMessage('Connexion réussie');
      // Vérifier si une URL de redirection est stockée
    const redirectUrl = sessionStorage.getItem('redirectAfterLogin');
    if (redirectUrl) {
      sessionStorage.removeItem('redirectAfterLogin'); // Supprimer l'URL après utilisation
      router.push(redirectUrl); // Rediriger vers l'URL de redirection
    } else {
      // Rediriger par défaut
