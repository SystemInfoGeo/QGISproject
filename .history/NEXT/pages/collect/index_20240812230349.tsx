...
export default function Collect() {
  ...
  const handleCloseMessage = async () => {
    if (data) {
      console.log('Sending data to /save-path:', data); // Log des données envoyées
      ...
      try {
        ...
        if (response.ok) {
          console.log('Requête POST envoyée à /save-path avec succès'); // Log de la requête réussie
        } else {
          console.error('Erreur lors de la sauvegarde du chemin:', response.statusText);
        }
      } catch (error) {
