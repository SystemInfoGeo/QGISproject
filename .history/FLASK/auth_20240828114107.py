...
@jwt_required()  # Assurez-vous que seul un administrateur peut accéder à cette route
def admin_register():
    current_user = get_jwt_identity()
    if not current_user.is_admin:  # Assurez-vous que l'utilisateur connecté est un administrateur
    ...
    if request.method == 'POST':
        ...
        last_name = request.form.get('last_name')
        phone_number = request.form.get('phone_number')
        role = request.form.get('role')  # Administrateur choisit le rôle
        ...
        new_user = User(
            ...
            last_name=last_name,
            phone_number=phone_number,
            role=role  # Rôle défini par l'administrateur
        )
    ...
    return render_template('admin_register.html')  # Page pour l'administrateur
