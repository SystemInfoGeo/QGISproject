...
def register():
    ...
    if 'email' not in data or 'password' not in data or 'first_name' not in data or 'last_name' not in data or 'phone_number' not in data:
        print("Clés manquantes dans la requête d'inscription.")
        return jsonify({'message': 'Clés manquantes.'}), 400

    email = data['email']
    password = data['password']
    confirm_password = data.get('confirm_password')  # Ajouter la confirmation du mot de passe
