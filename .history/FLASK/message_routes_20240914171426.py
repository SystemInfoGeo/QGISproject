...
def list_messages():
    try:
        ...
        print("Messages récupérés:", messages)  # Log des messages récupérés
        messages_data = [
            {
                "id": message.id,
                "name": message.name,
        ...
        print("Données renvoyées:", messages_data)  # Log des données renvoyées à React
        return jsonify({
...
def send_email(to_email, subject, message):
    ...
    msg['To'] = to_email
    msg['Subject'] = subject
    msg.attach(MIMEText(message, 'plain', 'utf-8'))  # Utilisation de l'encodage UTF-8
