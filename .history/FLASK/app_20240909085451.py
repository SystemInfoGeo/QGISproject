...
def user_profile():
    try:
        ...
        print(f"Identité JWT obtenue: {current_user}")  # Log pour débogage

        # Vérifier que le JWT contient bien l'email
        if isinstance(current_user, dict) and 'email' in current_user:
            current_user_email = current_user['email']
        else:
            print("Erreur : JWT ne contient pas d'email valide.")
            return jsonify({'error': 'Token JWT invalide'}), 400

        print(f"Utilisateur connecté : {current_user_email}")
        ...
        if user:
            ...
            print(f"Erreur : Utilisateur non trouvé pour l'email {current_user_email}")
