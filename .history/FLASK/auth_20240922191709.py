...
def login():
    ...
    # Créer un access_token avec seulement l'email
    access_token = create_access_token(identity=user.email)

    # Créer une réponse pour envoyer un cookie sécurisé avec le token
    response = make_response(jsonify({"message": "Connexion réussie!", "role": user.role}))
    
    # Ajouter le token dans un cookie HttpOnly (inaccessible via JavaScript) et Secure
