...
def admin_page():
    try:
        ...
        print(f"Utilisateur actuel : {current_user}")  # Log

        # Rechercher l'utilisateur dans la base de données
        user = User.query.filter_by(email=current_user).first()
        print(f"Utilisateur trouvé : {user}")  # Log

        # Vérifier si l'utilisateur a le rôle d'admin
        if user and user.role == 'admin':
            print("L'utilisateur est admin")  # Log
            return jsonify({"message": "Bienvenue sur la page d'administration."}), 200
        else:
            print("Accès refusé : L'utilisateur n'est pas admin")  # Log
            return jsonify({"error": "Accès refusé. Vous n'avez pas les droits d'administration."}), 403
    except Exception as e:
        print(f"Erreur lors de la vérification de l'utilisateur : {str(e)}")  # Log de l'erreur
