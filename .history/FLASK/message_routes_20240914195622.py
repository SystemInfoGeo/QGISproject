...
def list_messages():
    try:
        logger.info("Début de la récupération des messages")
        messages = Message.query.all()

        if not messages:
            logger.warning("Aucun message trouvé dans la base de données")
        else:
            logger.info(f"Messages récupérés : {messages}")
        ...
        logger.info(f"Messages à renvoyer : {messages_data}")

        logger.info("Données des messages renvoyées à l'API")
        return jsonify({
            "messages": messages_data,  # Assurez-vous d'envoyer un tableau de messages ici
            "pending_messages": len([msg for msg in messages if not msg.reply]),  # Messages en attente
            "total_messages": len(messages)  # Total des messages
        }), 200
    except SQLAlchemyError as e:
        logger.error(f"Erreur SQL lors de la récupération des messages: {str(e)}")
...
def send_email(to_email, subject, message):
    ...
    password = os.getenv('SMTP_PASSWORD')  # Stockez le mot de passe dans une variable d'environnement
