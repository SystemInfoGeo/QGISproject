...
# Optionnal: Use TSP algorithm for optimal path calculation
def calculate_optimal_path(graph, start_node_index, end_node_index, positions):
    try:
        from networkx.algorithms import approximation
        cycle = approximation.traveling_salesman_problem(graph, cycle=False)
        optimal_path = [positions[node] for node in cycle]
        return optimal_path
    except Exception as e:
        print(f"An error occurred: {e}")
        return None
