...
def login():
    ...
    # Répondre avec le token, le rôle et les informations de l'utilisateur
    return jsonify({
        "message": "Connexion réussie!",
        "token": token,  # Retourne "token" ici
        "role": user.role,
        "userInfo": {  
            "email": user.email
        }
...
def logout():
    ...
    logging.info("Déconnexion réussie.")  # Log général sans détail sur l'utilisateur
    return response, 200  
