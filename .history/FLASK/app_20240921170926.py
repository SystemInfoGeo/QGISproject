...
ORS_API_KEY = os.getenv('5b3ce3597851110001cf62483c4e70786f7c460498f73f7894803c2c')  # Clé API OpenRouteService
client = openrouteservice.Client(key="5b3ce3597851110001cf62483c4e70786f7c460498f73f7894803c2c"  )

# Point de départ et point d'arrivée fixes pour le camion
Point_depart_fixe = {'latitude': 36.714666886023693, 'longitude': 4.045495309895148}
Point_arrivee_fixe = {'latitude': 36.706130742911107, 'longitude': 4.012404383608166}
...
# Fonction pour calculer le chemin optimal avec TSP
...
def calculate_route():
    ...
    try:
        # Filtrer uniquement les points avec poubelles pleines (status == 'plein')
        points = [{'latitude': coord[1], 'longitude': coord[0], 'status': coord[2]} for coord in coordinates]
        points_pleins = [point for point in points if point['status'] == 'plein']

        # Ajouter le point de départ et d'arrivée fixes
        points_pleins = [Point_depart_fixe] + points_pleins + [Point_arrivee_fixe]

        # Construire le graphe des points pleins
        graph, positions = build_graph(points_pleins)

        # Calculer le chemin optimal
        start_node_index = 0
        end_node_index = len(points_pleins) - 1
...
def user_profile():
    try:
        ...
        logging.error(f"Erreur lors de l'exécution de la route : {str(e)}")
