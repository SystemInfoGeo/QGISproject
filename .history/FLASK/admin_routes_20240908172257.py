...
@jwt_required()
def admin_profile():
    print("Requête reçue sur /admin/profile")  # Log pour confirmer que la route est atteinte
    try:
        current_user = get_jwt_identity()
        print(f"Utilisateur connecté via JWT : {current_user}")  # Log pour voir l'identité de l'utilisateur via JWT
        
        if not current_user:
            print("Erreur : aucun utilisateur trouvé pour le token.")
            return jsonify({'error': 'Token invalide ou expiré'}), 401
        
        # Assurez-vous que le `current_user` contient bien un email
        print(f"Email reçu via JWT : {current_user}")  # Log pour voir ce que contient `current_user`

        admin = User.query.filter_by(email=current_user).first()
        print(f"Administrateur trouvé : {admin}")  # Log pour vérifier que l'administrateur est trouvé
