...
def admin_register():
    ...
    if not current_user.is_admin:  
        # Si l'utilisateur n'est pas un administrateur, renvoie un message d'accès non autorisé et redirige vers la page de connexion
        flash('Accès non autorisé', 'danger')
        return redirect(url_for('auth.login'))

    # Si la méthode de requête est POST, cela signifie que le formulaire d'inscription a été soumis
    if request.method == 'POST':
        ...
        last_name = request.form.get('last_name')
        phone_number = request.form.get('phone_number')
        role = request.form.get('role')  # L'administrateur choisit le rôle de l'utilisateur
        ...
        user = User.query.filter_by(email=email).first()
        if user:
            # Si l'utilisateur existe déjà, affiche un message d'erreur et redirige vers le formulaire d'inscription
        ...
        # Affiche un message de succès et redirige vers le formulaire d'inscription pour ajouter un autre utilisateur si nécessaire
        flash(f'{role.capitalize()} ajouté avec succès!', 'success')
        return redirect(url_for('auth.admin_register'))

    # Si la méthode de requête est GET, affiche la page de formulaire pour l'administrateur
    return render_template('admin_register.html')  # Page HTML pour l'administrateur
