...
Pour ajouter une fonctionnalité qui permet de récupérer tous les points de collecte depuis la base de données, ainsi que des boutons pour modifier ou supprimer chaque point, vous devez effectuer les étapes suivantes dans votre application. Voici comment procéder :

1. Backend Flask : Créer des routes pour récupérer, modifier et supprimer les points
Route pour récupérer tous les points de collecte
Cette route renvoie tous les points de collecte enregistrés dans la base de données.

python
Copier le code
@app.route('/get-all-trash-bins', methods=['GET'])
