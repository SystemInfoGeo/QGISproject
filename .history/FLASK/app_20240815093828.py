...
def save_path():
    global saved_data, data_saved
    data = request.json
    print("Données reçues pour la sauvegarde:", data) 
    ...
    try:
        ...
        db.session.commit()
        logger.debug("Chemin optimal sauvegardé dans la base de données")
...
def reset_qgis_points():
    print("Fonction reset_qgis_points appelée")
...
# Ajout de la nouvelle route pour récupérer les chemins optimaux
@app.route('/get_optimal_paths', methods=['GET'])
def get_optimal_paths():
    try:
        # Récupérer tous les chemins optimaux depuis la base de données
        paths = Path.query.all()

        # Transformer chaque chemin optimal en un dictionnaire pour le format JSON
